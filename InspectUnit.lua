...
local function GetPlayerSpec()
    if (player_spec) then return player_spec end

    player_spec = {}

    player_spec.idx, player_spec.talent = LibSpecs.GetSpecializationTalent()
    player_spec.id, player_spec.name = LibSpecs.GetSpecializationInfo(player_spec.idx)
...
--顯示面板
function ShowInspectItemListFrame(unit, parent, ilevel, maxLevel, spec)
...
LibEvent:attachTrigger("UNIT_INSPECT_READY, UNIT_REINSPECT_READY", function(self, data)
    ...
    if (InspectFrame and InspectFrame.unit and UnitGUID(InspectFrame.unit) == data.guid) then
        local frame = ShowInspectItemListFrame(InspectFrame.unit, InspectFrame, data.ilevel, data.maxLevel, data.spec)
...
LibEvent:attachTrigger("INSPECT_FRAME_COMPARE", function(self, frame)
    ...
    if (MerInspectDB and MerInspectDB.ShowOwnFrameWhenInspecting) then
        ...
        local playerFrame = ShowInspectItemListFrame("player", frame, ilevel, maxLevel, GetPlayerSpec())
...
PaperDollFrame:HookScript("OnShow", function(self)
    if (MerInspectDB and MerInspectDB.ShowCharacterItemSheet) then
        local ilevel, _, maxLevel = LibItemInfo:GetUnitItemLevel("player")
        ShowInspectItemListFrame("player", self, ilevel, maxLevel, GetPlayerSpec())
    end
...
LibEvent:attachEvent("PLAYER_EQUIPMENT_CHANGED", function(self)
    if (CharacterFrame:IsShown() and PaperDollFrame:IsShown() and MerInspectDB and MerInspectDB.ShowCharacterItemSheet) then
        local ilevel, _, maxLevel = LibItemInfo:GetUnitItemLevel("player")
        ShowInspectItemListFrame("player", PaperDollFrame, ilevel, maxLevel, GetPlayerSpec())
    end
