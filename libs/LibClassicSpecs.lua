local MAJOR, MINOR = "LibClassicSpecs-Mod", 1000 + tonumber(strmatch("@project-revision@", "%d+") or 0)
local lib = LibStub:NewLibrary(MAJOR, MINOR)

-- Credit: https://github.com/tstirrat/LibClassicSpecs
...
local function GetUnitClass(isInspect)
  local unit = isInspect and InspectFrame.unit or "player"

  return UnitClass(unit)
end

local function GetSpecializationTalent(isInspect, isPet, groupId)
  if isPet then
    return nil
  end

  if (isInspect and not (InspectFrame and InspectFrame.unit)) then
    return nil
  end
  ...
  local specIndex
  local max = 0
  for tabIndex = 1, GetNumTalentTabs(isInspect) do
    local spent = select(3, GetTalentTabInfo(tabIndex, isInspect))
    talent[tabIndex] = spent
    if (spent > max) then
      specIndex = tabIndex
      max = spent
    end
  ...
  local talentString = table.concat(talent, '/')

  local classId = select(3, GetUnitClass(isInspect))
  if (classId == Druid.ID) then
    -- Return Druid.Guardian if certain talents are selected
    local feralInstinctPoints = select(5, GetTalentInfo(DRUID_FERAL_TAB, DRUID_FERAL_INSTINCT, isInspect))
    local thickHidePoints = select(5, GetTalentInfo(DRUID_FERAL_TAB, DRUID_THICK_HIDE, isInspect))
    if (feralInstinctPoints == 3 and thickHidePoints == 3) then
      return DRUID_GUARDIAN_SPEC_INDEX, talentString
    end

    -- return 4 if Resto (3rd tab has most points), because Guardian is 3
    if (specIndex == DRUID_GUARDIAN_SPEC_INDEX) then
      return DRUID_RESTO_SPEC_INDEX, talentString
    else
      return specIndex, talentString
    end
  end
...
lib.GetSpecializationTalent = GetSpecializationTalent

lib.GetSpecialization = function(isInspect, isPet, groupId)
  local specIdx = GetSpecializationTalent(isInspect, isPet, groupId)
  return specIdx
end
...
lib.GetSpecializationInfo = function(specIndex, isInspect, isPet, inspectTarget, sex)
  ...
  local _, className = GetUnitClass(isInspect)
...
lib.GetSpecializationInfoForClassID = function(classId, specIndex)
  ...
  local isAllowed = classId == select(3, GetUnitClass(isInspect))
