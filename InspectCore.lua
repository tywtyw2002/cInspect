...
LibEvent:attachEvent("INSPECT_READY", function(this, guid)
    ...
    LibSchedule:AddTask({
        ...
        onExecute = function(self)
            ...
            self.data.spec = {}
            local specIdx, talent = LibSpecs.GetSpecializationTalent(true)
            self.data.spec.id, self.data.spec.name = LibSpecs.GetSpecializationInfo(specIdx, true)
            self.data.spec.class = self.data.class
            self.data.spec.race = select(2, UnitRace(self.data.unit))
            self.data.spec.level = UnitLevel(self.data.unit)
            self.data.spec.idx = specIdx
            self.data.spec.talent = talent
